...
func New(pool *pgxpool.Pool, imgStore *imagestore.Store, verifier *oidc.IDTokenVerifier, webDist fs.FS) (http.Handler, error) {
	...
	handlers := catch.NewHandlers(repo, imgStore)
	requireAuth := authmw.RequireAuth(verifier)

	mux := http.NewServeMux()

	mux.HandleFunc("GET /api/catches", withMiddleware(handlers.List))
	mux.HandleFunc("GET /api/catches/{id}", withMiddleware(handlers.Get))
